 ...
 * Time: 11:09 AM
 */
class Facebook_login extends REST_Controller {
    ...
    public function index_post(){
        ...
        $facebookConn = $this->parent_facebook_model->getByCode($code);

        if(empty($facebookConn)){
            if($this->form_validation->run('parent.signin.facebook') === false){
            ...
            $parentFacebookData->facebookAuthCode = $this->post('code');
            $newParentFacebook = $this->parent_facebook_model->save($parentFacebookData);
            ...
            $respond->user = 'new';
            $respond->email = $user->getEmail();
            $this->response($respond);

        }

        $parent = $this->parent_model->getByParentId($facebookConn->getParentId());
        $respond = new stdClass();
