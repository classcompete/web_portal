...
class Reportlib{
    ...
    public function __construct(){
        ...
        $this->ci->load->model('y_reporting/report_model');
        $this->ci->load->helper('y_reporting/report');
...
//        $number_of_question_in_challenge = $this->ci->question_model->count_questions_in_challenge($challenge_id);
    ...
    /** function for getting average challenge result */
    public function get_challenge_average_result($challenge_id){
        $classes = $this->ci->report_model->get_all_classes_with_challenge($challenge_id);
        $number_of_question_in_challenge = 0;
        $class_id_list = array();
        foreach($classes as $class=>$val){
            $class_id_list[] = $val->getClassId();
