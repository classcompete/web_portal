...
class Studentimportlib {
	...
	public function import_smart($importId) {
		...
		if ($ext == 'csv') { return $this->import_csv($importId); }
		else if (($ext == 'xls') || ($ext == 'xlsx')) { return $this->import_xls($importId); }
	}
		        ...
		        if ($totalRows++) {
			        ...
			        if ($impFirstName && $impLastName && $impGrade && $impGender && $impUsername && $impPassword) {
				        ...
				        if ($grade_ok) {
					        ...
					        if ($gradeObj) { $impGradeId = $gradeObj->getId(); }
					        else { $grade_ok = FALSE; }
				        }
				        ...
				        if (!$impFirstName) { $errors[] = '* No first name data in row ' . $totalRows; }
				        else if (!$impLastName) { $errors[] = '* No last name data in row ' . $totalRows; }
				        else if (!$impGrade) { $errors[] = '* No grade data in row ' . $totalRows; }
				        else if (!$impGender) { $errors[] = '* No gender data in row ' . $totalRows; }
				        else if (!$impUsername) { $errors[] = '* No username data in row ' . $totalRows; }
				        else if (!$impPassword) { $errors[] = '* No password data in row ' . $totalRows; }
			        }
		        }
	        }
		} catch (Exception $e) {
				...
				if ($totalRows++) {
					...
					if ($impFirstName && $impLastName && $impGrade && $impGender && $impUsername && $impPassword) {
						...
						if ($grade_ok) {
							//Find grade ID
							$tmpGrade = strtolower($impGrade);
							if (($tmpGrade != 'pre k') && ($tmpGrade != 'k')) { $tmpGrade = 'grade ' . $tmpGrade; }
							$gradeObj = $this->ci->grade_model->getGradeByName($tmpGrade);
							if ($gradeObj) { $impGradeId = $gradeObj->getId(); }
							else { $grade_ok = FALSE; }
						}
						...
						else if (! $impFirstName) {	$errors[] = '* No first name data in row ' . $totalRows; }
						else if (! $impLastName) { $errors[] = '* No last name data in row ' . $totalRows;	}
						else if (! $impGrade) { $errors[] = '* No grade data in row ' . $totalRows; }
						else if (! $impGender) { $errors[] = '* No gender data in row ' . $totalRows; }
						else if (! $impUsername) { $errors[] = '* No username data in row ' . $totalRows; }
