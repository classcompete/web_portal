...
jQuery(function ($) {
    ...
    $(' .isotope > li ').each(function () {
        ...
        if ($this.parent('ul').hasClass('challenge-list') === false) {
    ...
    // this is single challenge trigger
    jQuery('.cc_challenge').unbind('click').click(function (e) {
        e.preventDefault();
        console.log('asdasdasd');
        $('body').append('<div class="modal-backdrop fade in"></div>');

        var t = $(this);
        var modal = t.siblings('.tooltip_content');
        var modal_content = modal.html();
//        var avatar_clone = t.children('img').attr('src');

//        t.siblings('.tooltip_content').children('.tooltip_author div .tooltip_img').attr('src', avatar_clone);

        $('#modalbox .modal-body').html(modal_content);
        $('#modalbox').modal('show');
    });
