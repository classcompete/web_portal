<?php
if (!class_exists('WPPostsRateKeys_ExternalCloackedLinks')) {
	class WPPostsRateKeys_ExternalCloackedLinks
	{
		/**
		 ...
		 */
		static public function get_common_query($keywords='',$cloaking_folder='',$external_url='',$how_many='',$not_condition=0) {
			...
			if ($cloaking_folder!='') {
				$where_conditions[] = " cloaking_folder like %s";
				$query_params[] = "%$cloaking_folder%";
			}
			if ($external_url!='') {
				$where_conditions[] = " external_url like %s";
				$query_params[] = "%$external_url%";
			}
			if ($how_many!='') {
		 ...
		 */
		static public function search($keywords='',$cloaking_folder='',$external_url='',$how_many=''
										,$not_condition=0
			...
			if (count($fields)==0) {
				...
				if (in_array('cloaking_folder',$fields)) {
					$query .= ", cloaking_folder";
				}
				if (in_array('external_url',$fields)) {
					$query .= ", external_url";
				}
			...
			// Add common query
			$query_common_data = self::get_common_query($keywords,$cloaking_folder,$external_url,$how_many,$not_condition,$fields);
			$query .= $query_common_data[0];
			
			// Pagination and Order
			if ($orderby=='times_to_link') $orderby = 'how_many';
			if ($orderby!='') {
		 ...
		 * @return 	int
		 */
		static public function search_count($keywords='',$cloaking_folder='',$external_url='',$how_many=''
										,$not_condition=0) {
		   	global $wpdb;
			$table =  $wpdb->prefix . WPPostsRateKeys::$db_prefix . self::$table_name;
			...
			// Add common query
			$query_common_data = self::get_common_query($keywords,$cloaking_folder,$external_url,$how_many,$not_condition); 
			$query .= $query_common_data[0];
		...
		static public function update($data,$where)
		{
		   	self::set_db_obj();
		   	if (self::$db_obj!=NULL) {
	   			WPPostsRateKeys_Settings::update_external_link_modification_time(time());
